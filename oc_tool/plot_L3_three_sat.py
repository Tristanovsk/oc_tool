...
for year in range(2002, 2012):
    ...
    if not os.path.exists(odirfig):
        os.makedirs(odirfig)
    for doy in np.arange(182, 267, 8):

        date = str(year) + '_' + str(doy)
        file = '[ATV]' + date + 'to*.nc'
        file_a = glob.glob(opj(projet, 'satellite', sats[0], dirL3, str(year), file))
        file_t = glob.glob(opj(projet, 'satellite', sats[1], dirL3, str(year), file))
        file_v = glob.glob(opj(projet, 'satellite', sats[2], dirL3, str(year), file))
        print(file_a, file_t, file_v)

        date = datetime.datetime.strptime(date, '%Y_%j')
        ...
        for i, file in enumerate([file_a, file_t, file_v]):
            ...
            p = ds[param].where(ds[param] > 0).plot(ax=ax, transform=crs, cmap=cmap,
                                                    cbar_kwargs=dict(pad=.1, aspect=20, shrink=0.9))
            p.set_clim(0, 1)
            plt.title(date)
